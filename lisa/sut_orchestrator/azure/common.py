...
def get_or_create_file_share(
    ...
    if file_share_name not in (x.name for x in all_shares):
        ...
        try:
            ...
            # Handle case where PV2 parameters are not supported by the storage account
            if "UnsupportedHeader" in str(e) and (
                ...
                if create_kwargs.get("quota", 0) < 100:
                    log.warning(
                        f"Increasing quota from {create_kwargs.get('quota')} to "
                        "(100 GiB minimum for non-PV2 premium file shares)"
                    )
